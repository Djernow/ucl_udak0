...
cp -v "$SCRIPT_DIR/icon-192.svg" "$DATASET_PATH/"
cp -v "$SCRIPT_DIR/icon-maskable-192.svg" "$DATASET_PATH/"
cp -v "$SCRIPT_DIR/icon-512.svg" "$DATASET_PATH/"
...
if [ -d "$SCRIPT_DIR/badges" ]; then
    echo "📋 Copying badges directory..."
    mkdir -p "$DATASET_PATH/badges"
    cp -rv "$SCRIPT_DIR/badges/"* "$DATASET_PATH/badges/"
fi
...
chmod 644 "$DATASET_PATH/icon-192.svg"
chmod 644 "$DATASET_PATH/icon-maskable-192.svg"
chmod 644 "$DATASET_PATH/icon-512.svg"
...
if [ -d "$DATASET_PATH/badges" ]; then
    chmod 755 "$DATASET_PATH/badges"
    chmod 644 "$DATASET_PATH/badges/"*
fi
